...
class ExposedPodsHandler(Vulnerability, Event):
    ...
    def __init__(self, pods):
        Vulnerability.__init__(
            self, Kubelet, "Exposed Pods", category=InformationDisclosure
        )
        self.pods = pods
...
class ExposedPortForwardHandler(Vulnerability, Event):
    ...
    def __init__(self):
        Vulnerability.__init__(
            self, Kubelet, "Exposed Port Forward", category=RemoteCodeExec, vid="KHV041"
        )
...
class PrivilegedContainers(Vulnerability, Event):
    ...
    def __init__(self, containers):
        ...
        self.evidence = "pod: {}, container: {}, count: {}".format(
            containers[0][0], containers[0][1], len(containers)
        )
...
class KubeletHandlers(Enum):
    ...
    RUN = (
        "run/{pod_namespace}/{pod_id}/{container_name}?cmd={cmd}"
...
class ReadOnlyKubeletPortHunter(Hunter):
    ...
    def find_privileged_containers(self):
        logging.debug(
            "Passive hunter is attempting to find privileged containers and their pods"
        )
    ...
    def execute(self):
        ...
        if self.pods_endpoint_data:
            self.publish_event(
                ExposedPodsHandler(pods=self.pods_endpoint_data["items"])
            )
...
class SecureKubeletPortHunter(Hunter):
    ...
    class DebugHandlers(object):
        ...
        def test_exec_container(self):
            ...
            return (
                "/cri/exec/"
                in self.session.get(
                    exec_url, headers=headers, allow_redirects=False, verify=False
                ).text
            )
        ...
        def test_port_forward(self):
            ...
            self.session.get(
                pf_url, headers=headers, verify=False, stream=True
            ).status_code == 200
            # TODO: what to return?
        ...
        def test_attach_container(self):
            ...
            return (
                "/cri/attach/"
                in self.session.get(
                    attach_url, allow_redirects=False, verify=False
                ).text
            )
        ...
        def test_pprof_cmdline(self):
            cmd = self.session.get(
                self.path + KubeletHandlers.PPROF_CMDLINE.value, verify=False
            )
    ...
    def test_handlers(self):
        ...
        if pod:
            debug_handlers = self.DebugHandlers(
                self.path, pod=pod, session=self.session
            )
            try:
                ...
                if running_pods:
                    self.publish_event(
                        ExposedRunningPodsHandler(count=len(running_pods["items"]))
                    )
    ...
    def get_random_pod(self):
        if self.pods_endpoint_data:
            ...
            is_default_pod = (
                lambda pod: pod["metadata"]["namespace"] == "default"
                and pod["status"]["phase"] == "Running"
            )
            is_kubesystem_pod = (
                lambda pod: pod["metadata"]["namespace"] == "kube-system"
                and pod["status"]["phase"] == "Running"
            )
            pod_data = next(
                (pod_data for pod_data in pods_data if is_default_pod(pod_data)), None
            )
            if not pod_data:
                pod_data = next(
                    (pod_data for pod_data in pods_data if is_kubesystem_pod(pod_data)),
                    None,
                )
...
class ProveRunHandler(ActiveHunter):
    ...
    def execute(self):
        pods_raw = self.event.session.get(
            self.base_path + KubeletHandlers.PODS.value, verify=False
        ).text
...
class ProveContainerLogsHandler(ActiveHunter):
    ...
    def __init__(self, event):
        ...
        self.base_url = "{protocol}://{host}:{port}/".format(
            protocol=protocol, host=self.event.host, port=self.event.port
        )

    def execute(self):
        pods_raw = self.event.session.get(
            self.base_url + KubeletHandlers.PODS.value, verify=False
        ).text
        if "items" in pods_raw:
            pods_data = json.loads(pods_raw)["items"]
            for pod_data in pods_data:
                ...
                if container_data:
                    ...
                    if output.status_code == 200 and output.text:
                        self.event.evidence = "{}: {}".format(
                            container_data["name"], output.text.encode("utf-8")
                        )
                        return
