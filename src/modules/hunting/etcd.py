...
class EtcdRemoteAccessActive(ActiveHunter):
    ...
    def db_keys_write_access(self):
        logging.debug(
            "Active hunter is attempting to write keys remotely on host "
            + self.event.host
        )
        data = {"value": "remotely written data"}
        try:
            r = requests.post(
                "{protocol}://{host}:{port}/v2/keys/message".format(
                    host=self.event.host, port=2379, protocol=self.protocol
                ),
                data=data,
            )
            self.write_evidence = (
...
class EtcdRemoteAccess(Hunter):
    ...
    def db_keys_disclosure(self):
        logging.debug(
            self.event.host + " Passive hunter is attempting to read etcd keys remotely"
        )
        try:
            r = requests.get(
                "{protocol}://{host}:{port}/v2/keys".format(
                    protocol=self.protocol, host=self.event.host, port=2379
                ),
                verify=False,
            )
            self.keys_evidence = (
    ...
    def version_disclosure(self):
        logging.debug(
            self.event.host
            + " Passive hunter is attempting to check etcd version remotely"
        )
        try:
            r = requests.get(
                "{protocol}://{host}:{port}/version".format(
                    protocol=self.protocol, host=self.event.host, port=2379
                ),
                verify=False,
            )
    ...
    def insecure_access(self):
        logging.debug(
            self.event.host + " Passive hunter is attempting to access etcd insecurely"
        )
        try:
            r = requests.get(
                "http://{host}:{port}/version".format(host=self.event.host, port=2379),
                verify=False,
            )
