...
class CveUtils:
    ...
    def version_compare(v1, v2):
        ...
        v1_raw, v2_raw = (
            CveUtils.to_raw_version(v1).strip("v"),
            CveUtils.to_raw_version(v2).strip("v"),
        )
...
class K8sClusterCveHunter(Hunter):
    ...
    def execute(self):
        logging.debug(
            "Api Cve Hunter determining vulnerable version: {}".format(
                self.event.version
            )
        )
        cve_mapping = {
        ...
        for vulnerability, fix_versions in cve_mapping.items():
            if CveUtils.is_vulnerable(
                fix_versions, self.event.version, config.ignore_downstream
            ):
...
class KubectlCVEHunter(Hunter):
    ...
    def execute(self):
        ...
        logging.debug(
            "Kubectl Cve Hunter determining vulnerable version: {}".format(
                self.event.version
            )
        )
        for vulnerability, fix_versions in cve_mapping.items():
