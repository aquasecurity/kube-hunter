...
class HTTPDispatcher(object):
    def dispatch(self, report):
        ...
        dispatch_url = os.environ.get(
            "KUBEHUNTER_HTTP_DISPATCH_URL", "https://localhost/"
        )
        try:
            r = requests.request(
                dispatch_method,
            ...
            logging.debug(
                "\tResponse Code: {status}\n\tResponse Data:\n{data}".format(
                    status=r.status_code, data=r.text
                )
            )
            ...
            logging.error(
                "\nCould not dispatch report using HTTP {method} to {url}\nResponse Code: {status}".format(
                    status=r.status_code, url=dispatch_url, method=dispatch_method
                )
            )
            ...
            logging.error(
                "\nCould not dispatch report using HTTP {method} to {url} - {error}".format(
                    method=dispatch_method, url=dispatch_url, error=e
                )
            )
