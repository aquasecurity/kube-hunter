...
class RunningAsPodEvent(Event):
    ...
    def get_service_account_file(self, file):
        try:
            with open(
                "/var/run/secrets/kubernetes.io/serviceaccount/{file}".format(file=file)
            ) as f:
                return f.read()
        except IOError:
            pass
...
class HostDiscoveryHelpers:
    ...
    def get_cloud(host):
        try:
            ...
            metadata = requests.get(
                "https://www.azurespeed.com/api/region?ipOrUrl={ip}".format(ip=host),
                verify=False,
            ).text
    ...
    def generate_subnet(ip, sn="24"):
        ...
        for ip in IPNetwork(subnet):
            logging.debug(
                "HostDiscoveryHelpers.generate_subnet yielding {0}".format(ip)
            )
            yield ip
...
class FromPodHostDiscovery(Discovery):
    ...
    def execute(self):
        ...
        if config.remote or config.cidr:
            ...
            for subnet in subnets:
                if (
                    self.event.kubeservicehost
                    and self.event.kubeservicehost
                    in IPNetwork("{}/{}".format(subnet[0], subnet[1]))
                ):
                    should_scan_apiserver = False
                logging.debug(
                    "From pod scanning subnet {0}/{1}".format(subnet[0], subnet[1])
                )
                for ip in HostDiscoveryHelpers.generate_subnet(
                    ip=subnet[0], sn=subnet[1]
                ):
                    self.publish_event(NewHostEvent(host=ip, cloud=cloud))
            if should_scan_apiserver:
                self.publish_event(
                    NewHostEvent(
                        host=IPAddress(self.event.kubeservicehost), cloud=cloud
                    )
                )
    ...
    def azure_metadata_discovery(self):
        ...
        for interface in machine_metadata["network"]["interface"]:
            ...
            logging.debug(
                "From pod discovered subnet {0}/{1}".format(
                    address, subnet if not config.quick else "24"
                )
            )
...
class HostDiscovery(Discovery):
    ...
    def execute(self):
        if config.cidr:
            ...
            for host in config.remote:
                self.publish_event(
                    NewHostEvent(host=host, cloud=HostDiscoveryHelpers.get_cloud(host))
                )
    ...
    def generate_interfaces_subnet(self, sn="24"):
        for ifaceName in interfaces():
            for ip in [
                i["addr"] for i in ifaddresses(ifaceName).setdefault(AF_INET, [])
            ]:
                if (
                    not self.event.localhost
                    and InterfaceTypes.LOCALHOST.value in ip.__str__()
                ):
                    continue
