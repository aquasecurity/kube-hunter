...
class AzureMetadataApiExposed(Vulnerability, Event):
    ...
    def __init__(self, versions_info):
        ...
        self.evidence = f"apiVersions: {','.join(self.versions.keys())}"
...
class AzureInstanceMetadataServiceDiscovery(Discovery):
    ...
    def execute(self):
        ...
        logging.debug("Trying to access IMDS (Azure Metadata Service) from pod")
        available_versions = AzureInstanceMetadataService.get_versions(network_timeout=config.network_timeout)
        
        if not available_versions:
            logging.debug("IMDS not available")
            return

        versions_info = dict()
        for version in available_versions:
            instance_data = AzureInstanceMetadataService.get_instance_data(api_version=version, network_timeout=config.network_timeout)
            if instance_data:
                logging.debug(f"Successfully extracted IMDS apiVersion {version} instance data")
                versions_info[version] = instance_data
        
        self.publish_event(AzureMetadataApi(versions=versions_info))
